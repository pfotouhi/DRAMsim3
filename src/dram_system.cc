...
bool JedecDRAMSystem::WillAcceptTransaction(uint64_t hex_addr,
                                            ...
                                            bool is_write) const {
    int channel = GetChannel(hex_addr);
    return ctrls_[channel]->WillAcceptTransaction(hex_addr, requester, is_write);
}

bool JedecDRAMSystem::AddTransaction(uint64_t hex_addr, uint64_t start, bool is_write) {
// Record trace - Record address trace for debugging or other purposes
    ...
    assert(ok);
    if (ok) {
        Transaction trans = Transaction(hex_addr, start, is_write);
...
bool JedecDRAMSystem::AddTransaction(uint64_t hex_addr, uint64_t start, 
		                     uint64_t requester, bool is_write) {
// Record trace - Record address trace for debugging or other purposes
#ifdef ADDR_TRACE
    address_trace_ << std::hex << hex_addr << std::dec << " "
                   << (is_write ? "WRITE " : "READ ") << clk_ << std::dec << requester
		   << std::endl;
#endif

    int channel = GetChannel(hex_addr);
    bool ok = ctrls_[channel]->WillAcceptTransaction(hex_addr, requester, is_write);

    assert(ok);
    if (ok) {
        Transaction trans = Transaction(hex_addr, start, requester, is_write);
        ctrls_[channel]->AddTransaction(trans);
    }
    last_req_clk_ = clk_;
    return ok;
...
bool IdealDRAMSystem::AddTransaction(uint64_t hex_addr, uint64_t start, bool is_write) {
    auto trans = Transaction(hex_addr, start, is_write);
