...
void RandomCPU::ClockTick() {
    ...
    if (get_next_) {
        memory_system_.AddTransaction(last_addr_, clk_, last_write_);
    }
    clk_++;
    return;
}
...
void StreamCPU::ClockTick() {
    ...
    if (!inserted_a_ &&
        ...
        memory_system_.AddTransaction(addr_a_ + offset_, clk_, false);
        inserted_a_ = true;
    }
    if (!inserted_b_ &&
        memory_system_.WillAcceptTransaction(addr_b_ + offset_, false)) {
        memory_system_.AddTransaction(addr_b_ + offset_, clk_, false);
        inserted_b_ = true;
    }
    if (!inserted_c_ &&
        memory_system_.WillAcceptTransaction(addr_c_ + offset_, true)) {
        memory_system_.AddTransaction(addr_c_ + offset_, clk_, true);
        inserted_c_ = true;
    }
