...
class HMCMemorySystem : public BaseDRAMSystem {
   public:
    ...
    bool WillAcceptTransaction(uint64_t hex_addr, uint64_t requester,
                               bool is_write) const override;
    bool AddTransaction(uint64_t hex_addr, uint64_t start, bool is_write) override;
    bool AddTransaction(uint64_t hex_addr, uint64_t requester,
		        uint64_t start, bool is_write) override;
